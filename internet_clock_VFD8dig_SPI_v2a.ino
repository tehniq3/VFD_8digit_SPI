 ...
 * updated for 8-digit VFD SPI display
 */
...
String intro = "         NTP clock v.2a on 8-digit VFD SPI display by niq_ro        ";

#define din D7
#define clk D5
#define cs  D8
#define Reset D1
#define en 0  // connected to 3.3V
...
int vfd_lengh = 8;  // number of digits
...
if (targetTime > millis())  // to show new info just if is necesary
  VFD_Fix(ora);
...
//ss3 = ss;
int y1 = ora.length()+vfd_lengh;  // leng of scrolling text
for (int a = 0; a <= y1; a++) 
{
  ss3 = ss + millis()/1000 - ss2;
  String ora1 = "";
  if (hh/10 == 0)
    ora1 = ora1 + " ";
   else
...
for (int b = 0; b < vfd_lengh; b++) 
{
  VFD_WriteOneChar(b, ora1[a+b]-48);
}
delay(ics/2);
}
...
//int y = ora.length()+vfd_lengh;  // leng of scrolling text
for (int a = 0; a <= y1-vfd_lengh; a++) 
{
  ss3 = ss + millis()/1000 - ss2;
  ss3 = ss3 -1; // small corection after scrolling
  String ora2 = "        ";
  if (hh/10 == 0)
    ora2 = ora2 + " ";
   else
...
for (int b = 0; b < vfd_lengh; b++) 
{
  VFD_WriteOneChar(b, ora2[a+b]-48);
}
delay(ics/2);
}
 }  // end data scrolling
